"""Сборка паспорта поверх готового бланка.

Бланк (assets/source/template.png) — это исходный макет, с которого снято всё
переменное. Подписи полей, заголовок, цитаты, пословица, рамка, флаг, золотая
печать и тамга остаются РОДНОЙ графикой макета: их не набрать заново тем же
шрифтом и той же печатной фактурой.

Здесь только то, что меняется от гостя к гостю и от тотема к тотему.
...
# Шрифт значений подбирался под машинописную антикву макета.
VALUE_FONT = "/System/Library/Fonts/Supplemental/Courier New Bold.ttf"

# Замерено по исходнику: (верх, низ) каждой строки значений; текст от x=700.
VALUE_ROWS = [
    (266, 306), (337, 368), (400, 431), (460, 493), (521, 555),
...
def _font(size: int) -> ImageFont.FreeTypeFont:
    return ImageFont.truetype(VALUE_FONT, size)
...
def render(data: Passport, photo: "pathlib.Path | Image.Image | None") -> Image.Image:
    ...
    illustration = GENERATED / f"{data.totem_id}.png"
    if illustration.exists():
        _place_animal(canvas, illustration)

    # --- девять значений ---
    values = [data.name] + [p[k] for k in FIELD_ORDER]
    for (top, bottom), value in zip(VALUE_ROWS, values):
        height = bottom - top
        font = _fitted(d, value, 1130 - 700, height * 1.62, 0.5)
        _tracked(d, (700, (top + bottom) / 2), value, font, INK, tracking=0.5, anchor="lm")

    # --- шапка и правая колонка ---
    ...
    _draw_stamp(canvas, p["stamp"], data.track)
    ...
    blob_rng = np.random.default_rng(zlib.crc32(data.totem_id.encode()))
    for _ in range(7):
    ...
    track_mask = signs.TRACKS[data.track]()
    seed = zlib.crc32(data.totem_id.encode())
    ...
    texture = signs.TEXTURES[data.texture](
