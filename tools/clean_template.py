...
ERASE = [
    *[(f"значение поля {i + 1}", (693, y0, x1, y1), 13)
    ...
    # Низ полосы замерен по хвосту строки, а не на глаз: при границе 684
    # последняя «E» оставалась на бланке и печаталась на каждом паспорте.
    # Ниже 700 не опускаться — на y≈765 идёт рамка, её стирать нельзя.
    ("координаты справа", (1854, 386, 1902, 712), 9),
    # Левая полоса: «ҰЛЫ ДАЛА • ЕРКІНДІК • ЖЕЛ • АТ» (читается снизу вверх).
    # Стихия и животное — от тотема, а на бланке они конские, и барс получал
    # паспорт с надписью «ветер • конь». Снимаем только этот хвост (АТ • ЖЕЛ,
    # y 288..414); «ЕРКІНДІК • ҰЛЫ ДАЛА» ниже верно для всех четырёх.
    ("левая полоса: стихия и животное", (72, 288, 112, 414), 9),
